...
while True:
    ...
    if message:
        ...
        if contents[0] == '1':
            if contents[1] == '0':
                battery_warning(int(contents[2]))
    ...
    if utime.ticks_diff(utime.ticks_ms(), -d) >= 150 or utime.ticks_diff(utime.ticks_ms(), -d) < 0:
        radio.send("0" + "," + "1" + "," + str(roll_target) + "," + str(pitch_target) + "," + str(height) + "," + str(arm) + "," + str(throttle_flag))
